...
<div class="h-full overflow-y-auto p-8">
  <div class="max-w-lg mx-auto space-y-6">
    <div class="text-center space-y-2">
      <h1 class="text-2xl font-bold text-(--fg1)">Welcome to Endara</h1>
      <p class="text-sm text-(--fg2)">Endara relays MCP servers to your AI tools.</p>
    </div>

    <!-- Relay URL info -->
    <div class="rounded-lg border border-(--border) bg-(--surface-alt) p-4 space-y-2">
      <h3 class="text-sm font-semibold text-(--fg1)">Connect your AI client</h3>
      <p class="text-xs text-(--fg2)">
        Point Claude Desktop or other MCP clients to the Endara relay:
      </p>
      <div class="flex items-center gap-2">
        <code class="flex-1 text-xs font-mono bg-(--surface) border border-(--border) rounded px-2 py-1.5 text-(--accent)">
          {RELAY_MCP_URL}
        </code>
        <button
      ...
      <p class="text-xs text-(--fg2)">
        SSE endpoint: <code class="font-mono text-(--accent)">{RELAY_SSE_URL}</code>
      </p>
    </div>

    <button
      class="w-full px-4 py-3 text-sm font-medium rounded-lg bg-(--accent) text-white hover:bg-(--accent-hover) transition-colors shadow-sm"
    ...
    <div class="rounded-lg border border-(--border) bg-(--surface-alt) p-4">
      <div class="flex items-start justify-between gap-4">
        <div>
          <div class="text-sm font-medium text-(--fg1)">Start on Login</div>
