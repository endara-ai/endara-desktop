...
<div class="h-full overflow-y-auto p-4 space-y-3">
  <input
    ...
    class="w-full px-3 py-1.5 text-sm rounded-lg border border-(--border) bg-(--surface) focus:outline-none focus:ring-2 focus:ring-(--accent)/40"
  ...
  {:else}
    {#each filteredCatalog as entry (entry.name)}
      <div
        class="w-full text-left p-3 rounded-lg border border-(--border) hover:bg-(--surface-hover) transition-colors cursor-pointer {entry.available ? '' : 'opacity-50'}"
      ...
      >
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <span class="px-1.5 py-0.5 text-[10px] font-medium rounded-full bg-(--surface-alt) text-(--fg2)">{entry.endpoint}</span>
            {#if !entry.available}
            ...
            {/if}
            <button
              class="p-0.5 rounded hover:bg-(--surface-hover)"
              onclick={(e) => { e.stopPropagation(); navigateToEndpoint(entry.endpoint); }}
              title="Jump to endpoint"
            >
              <svg class="w-4 h-4 text-(--fg2)" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              ...
              title="Show details"
            >
              <svg class="w-4 h-4 text-(--fg2) transition-transform {expandedTool === entry.name ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {#if entry.description}
          <div class="text-xs text-(--fg2) mt-1">{entry.description}</div>
        {/if}
        {#if expandedTool === entry.name}
          {#if entry.inputSchema}
            <div class="mt-2">
              <div class="text-xs font-medium text-(--fg2) mb-1">Input Schema</div>
              <pre class="p-2 text-xs font-mono bg-(--surface-alt) rounded overflow-x-auto">{JSON.stringify(entry.inputSchema, null, 2)}</pre>
            </div>
          {/if}
          {#if entry.annotations}
            <div class="mt-2">
              <div class="text-xs font-medium text-(--fg2) mb-1">Annotations</div>
              <pre class="p-2 text-xs font-mono bg-(--surface-alt) rounded overflow-x-auto">{JSON.stringify(entry.annotations, null, 2)}</pre>
            </div>
          {/if}
        {/if}
      </div>
    ...
    {#if filteredCatalog.length === 0}
      <div class="text-sm text-(--fg2) text-center py-6">No tools found</div>
    {/if}
  {/if}
</div>
