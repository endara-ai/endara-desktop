...
- **Grant sandbox file access visually** — pick the directories that JS-execution scripts may read from and write into (the relay's `write_dirs` allowlist) with a native folder picker in Settings; the sandbox's `readFile()` and `writeFile()` reject any path outside these directories.
...
- **Write directories** — A **Write directories** section in Settings manages the relay's `[relay] write_dirs` allowlist — the directories sandbox scripts may read from and write into via `readFile()` / `writeFile()`, both of which reject any path outside them. Add entries with a native folder picker, remove them with one click; changes are persisted to `config.toml` and pushed to the running relay without a restart
